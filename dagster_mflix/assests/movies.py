...
@asset(deps=["dlt_mongodb_embedded_movies","dlt_mongodb_comments"])
...
@asset(deps=["dlt_mongodb_embedded_movies","dlt_mongodb_comments"], partitions_def=monthly_partition)
def top_movies_by_month(context,snowflake: SnowflakeResource)->None:
    ...
    context.log.info(f"top_movies_by_month_context: {context}")
    query = f"""
        select
            movies.title,
            movies.released,
...
@asset(deps=["user_engagement"])
def top_movies_by_engagement(snowflake: SnowflakeResource):
    """
